...
Os recordaré que Valdelinares es el pueblo "más alto" de Teruel, seguido de Griegos, pero ¿y mi pueblo? Mi pueblo es [Pancrudo](https://es.wikipedia.org/wiki/Pancrudo), y es muuuuy alto, pero veámoslo en un gráfico.
...
Primero he cargar los datos de lindes municipales. No quiero liarme mucho así que cargaré el fichero que usaba hace uno/dos añitos. No lo voy a actualizar ni chequear.
...
Bueno, como veis mis conocimientos sobre gráficos espaciales (y gráficos en general) en R es limitado, pero estoy en ello. De hecho hace como año y medio hacia gráficos bastante más potentes, usé `ggmpap`  `leaflet` y `ggplot2`, pero todo se olvida, y en cuanto a manipulación de datos espaciales es que prefiero empezar por lo nuevo; that is, el pkg `sf`. `r emo::ji("sunglasses")`

<br>

**P.S:** (Bastante técnico, al menos para mi)

Resulta que yo quería haber puesto los 2 post sobre altitudes en uno sólo, pero resulta que pero cuando los juntaba, me daba problemas la datatable. Mirando por internet vi la solución [aquí](https://stackoverflow.com/questions/43594039/dt-package-not-working-with-blogdown-using-hugo-future-imperfect-theme), pero funciona sólo a medias. la verdad es que lo mejor de R es la comunidad de usuarios!!

La solución era, grabar la datatable como un objeto e insertarla en un iframe. Abajo está el código. 
